...
namespace Esprima
{
    public class JavaScriptParser
    {
        ...
        public JavaScriptParser(string code, ParserOptions options, Action<Node> _action)
        {
            if (code == null)
            {
        ...
        private ClassProperty ParseClassElement(ref bool hasConstructor)
        {
            ...
            else
            {
                ...
                switch(key.Type)
                {
                    case Nodes.Identifier: id = key.As<Identifier>().Name; break;
                    case Nodes.Literal: id = key.As<Literal>().StringValue; break;
                    default: id = ThrowArgumentOutOfRangeException<string>(nameof(key.Type), key.Type); break;
                };
